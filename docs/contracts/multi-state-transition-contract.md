...
8. `deep-interview -> ralplan` is evidence-gated: answered or handoff-cleared question obligations alone do not complete deep-interview, and Autopilot ralplan handoff requires tracker-backed native Architect, Scholastic, and Critic lane evidence rather than `codex_exec`/artifact-only approvals
