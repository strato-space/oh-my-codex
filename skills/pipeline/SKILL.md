...
- **ralplan**: Consensus planning (planner + architect + scholastic + critic). Skips only when both `prd-*.md` and `test-spec-*.md` planning artifacts already exist **and** durable consensus evidence records Architect approval, then Scholastic approval, then Critic approval. Plan/test-spec files alone are not consensus evidence. If any required review is missing, blocked, out of order, or non-approving, the stage remains in ralplan or fails with an explicit blocker/max-iteration outcome instead of progressing to execution. Carries any `deep-interview-*.md` spec paths forward for traceability.
